...
export class jlBuilder extends InstructionFactory {

	constructor() {
		let opts: InstructionLineOptions = {
			description: '', name: 'jump when less', supported: true, tag: /^(\s*jl\s+\w+)$/
		};
		super(opts);
	}
	...
	apply_parse(line: UnparsedLOC, parse: ParseState): InstructionFactoryApplyParseReturnType {
		try {
			...
			let instruction = new jl(param_1, line);
			return { line: { type: 'instruction', runtime: instruction, loc: line } };
		} catch {
			return { line: { type: 'invalid', message: 'cannot parse jl instruction', loc: line } };
		}
...
export class jl extends ExecutableLine {
	dest: string;
	...
	execute(_trace: RuntimeTrace, system: vSystem): undefined {
		console.debug(`jl ${this.dest}`);

		if (system.status_less_then) {
