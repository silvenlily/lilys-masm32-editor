...
import { PointerDataOperand, type RegisterDataOperand } from '$lib/AsmInterpreter/system/DataOperand';
...
export class Push extends ExecutableLine {
	...
	constructor(dest: string, line: UnparsedLOC) {
		super(line);
		this.dest = this.resolve_operand(dest, ['RegisterDataOperand', 'PointerDataOperand']) as any;
	}

	execute(trace: RuntimeTrace, system: vSystem): undefined {
		let value = this.dest.get(system, this.requested_variable_address_resolutions);
