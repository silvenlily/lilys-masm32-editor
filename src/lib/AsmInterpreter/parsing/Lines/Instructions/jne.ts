...
export class jneBuilder extends InstructionFactory {

	constructor() {
		let opts: InstructionLineOptions = {
			description: '', name: 'jump when not equal', supported: true, tag: /^(\s*jne\s+\w+)$/
		};
		super(opts);
	}
	...
	apply_parse(line: UnparsedLOC, parse: ParseState): InstructionFactoryApplyParseReturnType {
		try {
			...
			let instruction = new jne(param_1, line);
...
export class jne extends ExecutableLine {
	dest: string;
