...
export class jgeBuilder extends InstructionFactory {

	constructor() {
		let opts: InstructionLineOptions = {
			description: '', name: 'jump when greater or equal', supported: true, tag: /^(\s*jge\s+\w+)$/
		};
		super(opts);
	}
	...
	apply_parse(line: UnparsedLOC, parse: ParseState): InstructionFactoryApplyParseReturnType {
		try {
			...
			let instruction = new jge(param_1, line);
			return { line: { type: 'instruction', runtime: instruction, loc: line } };
		} catch {
			return { line: { type: 'invalid', message: 'cannot parse jge instruction', loc: line } };
		}
...
export class jge extends ExecutableLine {
	dest: string;
	...
	execute(_trace: RuntimeTrace, system: vSystem): undefined {
		console.debug(`jge ${this.dest}`);

		if (system.status_equal_to || system.status_greater_then) {
