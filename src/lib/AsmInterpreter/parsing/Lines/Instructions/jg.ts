...
export class jgBuilder extends InstructionFactory {

	constructor() {
		let opts: InstructionLineOptions = {
			description: '', name: 'jump when equal', supported: true, tag: /^(\s*jg\s+\w+)$/
		};
		super(opts);
	}
	...
	apply_parse(line: UnparsedLOC, parse: ParseState): InstructionFactoryApplyParseReturnType {
		try {
			...
			let instruction = new jg(param_1, line);
...
export class jg extends ExecutableLine {
	dest: string;
	...
	execute(_trace: RuntimeTrace, system: vSystem): undefined {
		console.debug(`jg ${this.dest}`);

		if (system.status_greater_then) {
